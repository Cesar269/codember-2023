...
let alfanumericos =/^[a-zA-Z0-9]+$/;
let correo = /^[a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]+$/;
// let correo = /^[a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]$/;
let texto = /^[a-zA-Z\s]+$/;

const listaReglas = lista.trim().split('\n');

let reglasValidas = []
let reglasInvalidas = []

listaReglas.forEach((item,i) => {
    const parametros = item.split(',')
    const id = parametros[0]
    const username = parametros[1]
    const mail = parametros[2]
    const age = parametros[3] ? !isNaN(parseFloat(parametros[3])) : true
    const location = parametros[4] ? texto.test(parametros[4]) : true

    if(alfanumericos.test(id) && alfanumericos.test(username) && correo.test(mail) && age && location){
        reglasValidas.push(username)
    }else{
        reglasInvalidas.push(username)
    }

});

console.log(reglasInvalidas)
const cadenaRespuesta = reglasInvalidas.map((item) => item.at(0)).join('')
