...
listaReglas.forEach((item) => {
    const parametros = item.split(' ')
    const min = parametros[0].split('-')[0]
    const max = parametros[0].split('-')[1]
    const caracter = parametros[1][0]
    const cadenaString = parametros[2]
    const cadenaArray = parametros[2].split('')
    let count = 0
    cadenaArray.forEach(item => {
        item == caracter ? count++ : count
    })
    if (count <= max & count >= min) {
        reglasValidas.push(cadenaString)
    }else{
        reglasInvalidas.push(cadenaString)
    }

});


console.log(reglasValidas)
console.log(reglasInvalidas)
console.log(reglasInvalidas[12])